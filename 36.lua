...
--  Показывает цены ТОЛЬКО для: Commons, Uncommons, Rares,
--  Legendaries, Godlies, Chromas, Vintages, Ancients
-- ============================================================
...
-- ===== ЛОКАЛЬНАЯ БАЗА (только разрешенные категории) =====
local LOCAL_PRICES = {
    -- Commons
    ...
    -- Rares
    ["Ice Knife"] = 3, ["Fire Knife"] = 3,
    -- Legendaries
    ["Night Blade"] = 5, ["Shadow Gun"] = 5,
    -- Godlies
    Luger = 28, Shark = 20, Heat = 10, Tides = 10, Fang = 10,
    Clockwork = 10, Chill = 10, Frostsaber = 10, Spider = 10,
    Seer = 3, RedSeer = 3, BlueSeer = 3, PurpleSeer = 3, OrangeSeer = 2, YellowSeer = 2,
    ["Golden"] = 50, ["Laser"] = 22, Amerilaser = 22,
    -- Chromas
...
-- ===== URL ТОЛЬКО РАЗРЕШЕННЫХ КАТЕГОРИЙ =====
local CATEGORY_URLS = {}
for _, category in ipairs(ALLOWED_CATEGORIES) do
    table.insert(CATEGORY_URLS, "https://supremevalues.com/mm2/" .. category)
end
...
local function fetchAllPrices()
    ...
    for _, url in ipairs(CATEGORY_URLS) do
        ...
        if prices and next(prices) then
            ...
            loaded = loaded + count
            local category = url:match("mm2/(.+)$") or "unknown"
...
local function updateMainStatus()
    ...
    if statusText then
        ...
        local itemsStr = totalItemsLoaded > 0 and totalItemsLoaded or "локальные"
        statusText.Text = "Статус: ВКЛ | Цены: " .. itemsStr .. " | Обн: " .. timeStr
    end
end
